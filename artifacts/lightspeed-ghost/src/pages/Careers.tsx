    ...
    <div className="min-h-screen bg-[#f7f9fb] text-[#191c1e] antialiased">
      ...
      <main className="max-w-4xl mx-auto px-6 py-14 sm:py-20 space-y-20">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={text} className="flex items-start gap-3">
                  <div className="w-7 h-7 rounded-lg bg-[#eceef0] border border-[#e0e3e5] flex items-center justify-center shrink-0 mt-0.5">
                    <Icon size={13} className="text-[#76777d]" />
                  </div>
                  <p className="text-[#45464d] text-sm leading-relaxed">{text}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="p-6 rounded-2xl bg-gradient-to-br from-blue-600/10 to-violet-600/10 border border-[#10b981]/20">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={label} className="flex items-center justify-between border-b border-[#eceef0] pb-3 last:border-0 last:pb-0">
                  <span className="text-[#76777d] text-xs">{label}</span>
                  <span className="text-[#45464d] text-xs font-medium">{value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {PERKS.map(({ emoji, label, sub }) => (
              <div key={label} className="flex items-start gap-3.5 p-5 rounded-xl bg-white border border-[#e0e3e5]">
                <span className="text-2xl shrink-0">{emoji}</span>
                <div>
                  <p className="text-[#191c1e] font-semibold text-sm mb-1">{label}</p>
                  <p className="text-[#76777d] text-xs leading-relaxed">{sub}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {OPEN_ROLES.map((role) => (
              <div key={role.title} className="p-6 rounded-2xl bg-white border border-[#e0e3e5]">
                <div className="flex items-start justify-between gap-4 flex-wrap mb-3">
                  <div>
                    <h3 className="text-lg font-bold text-[#191c1e]">{role.title}</h3>
                ...
                <p className="text-[#45464d] text-sm leading-relaxed mb-4">{role.description}</p>
        ...
        {/* Don't see your role */}
        <div className="p-8 rounded-2xl bg-white border border-[#e0e3e5] text-center">
          <h2 className="text-xl font-semibold mb-3">Don't see the right role?</h2>
          <p className="text-[#76777d] text-sm leading-relaxed mb-6 max-w-md mx-auto">
      ...
      <footer className="border-t border-[#eceef0] py-8 px-6 text-center">
        <p className="text-[#9a9aa1] text-xs">© {new Date().getFullYear()} Light Speed Ghost · 500 Oracle Pkwy, Redwood City, CA 94065 · <a href="mailto:info@lightspeedghost.com" className="hover:text-[#45464d]">info@lightspeedghost.com</a></p>
      </footer>
    </div>
  );
}
