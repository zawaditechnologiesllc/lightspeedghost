...
const faqs = [
  {
    ...
    a: "Light Speed Ghost is an AI-powered academic toolkit for students from high school through to PhD. Unlike general AI that writes from memory, the research-facing tools write from real academic papers pulled from 35 live databases (OpenAlex, Semantic Scholar, CrossRef, PubMed, Europe PMC, arXiv, CORE, DOAJ, ERIC, Zenodo and 25 more) — 10 billion+ indexed records. A free checker is built into the command box and open to everyone — no subscription and no card needed, because it never touches an AI model: one click runs AI detection, a real plagiarism scan against those 35 databases, and a professor-style Peer Review, with a downloadable PDF report. One subscription then unlocks six AI tools: Paper Writer, Outline Builder, Paper Revision, LightSpeed Humanizer (for a natural academic voice in your own words), STEM Solver with step-by-step working, and a Study Assistant that reads your own materials. For finance and current-events questions, answers are grounded in live data and a vetted registry of 1,000+ authoritative sources worldwide, so the AI cites real figures instead of inventing them.",
  },
  {
  ...
  {
    q: "What types of papers and data analysis can it handle?",
    a: "Papers: essays (argumentative, persuasive, narrative, expository), research papers, dissertations and theses, literature reviews, lab reports, case studies, reflective and reaction papers, annotated bibliographies, research proposals, discussion posts, methodology and results sections, and more — 35 paper types in total, from high-school to PhD level. Data analysis: upload a CSV or spreadsheet and the tools run real statistics — descriptive stats, Pearson correlations, regression-style relationships, trend and distribution analysis — and weave your actual numbers and findings into the write-up. There's a dedicated financial-statement analyser (ratios, IFRS/GAAP) too. So whether you'd otherwise reach for SPSS, R, Stata, Excel or Python for the numbers, you can bring your dataset here and get an analysis plus the written interpretation in one place.",
  },
  {
  ...
  },
  {
    ...
    a: "Fair skepticism. The difference is that we write from real papers, not from memory. Here is exactly what happens on every paper: (1) We simultaneously query 35 live academic databases — OpenAlex, CrossRef, PubMed, Semantic Scholar, Europe PMC, ERIC, Zenodo, arXiv, CORE, DOAJ, BASE, DataCite, OpenAIRE, PLOS, PubMed Central, bioRxiv, medRxiv, SSRN, HAL, EconBiz, DOAB, OAPEN, Unpaywall, ClinicalTrials.gov, WHO, DBLP, Papers with Code, NBER, World Bank, NCBI Bookshelf, Dryad, Figshare, Google Books, Internet Archive and Open Library — pulling over 10 billion records worth of real abstracts, ranked by citation count. Between them these aggregators index the scholarship published across JSTOR, Scopus, ProQuest, Elsevier, Springer, Wiley, Nature, Science, IEEE and ACM journals, so that literature surfaces too. Every source is indexed, peer-reviewed, and clickable. No fabricated citations with broken URLs. (2) If you upload a grading rubric, we extract only the A-grade / Distinction criteria and lock them as requirements before writing starts. No rubric? We apply a preset Grade A standard calibrated from Harvard, Oxford, Yale, Princeton, MIT, and Cambridge. (3) After the paper is written, we cross-check it against those criteria and run a targeted improvement pass if any gaps are found, targeting a minimum 92% grade. (4) A plagiarism gate measures similarity and rephrases any section above 8% before we send it to you. (5) The Humanizer refines anything that reads robotic into a natural, authentic academic voice in your own words. That is the pipeline on every single output.",
  },
  {
  ...
  },
  {
    ...
    a: "Yes. The Free plan is $0 forever — no card required. One click on 'Check my writing' runs everything at once: AI detection, readability, grammar and tone (instant, in your browser), a real plagiarism scan against 35 live academic databases, and a professor-style Peer Review with a grade estimate — plus a downloadable PDF report once you sign in. Your text is never sent to an AI model on the Free plan; all of it runs on local, statistical detection. For AI paper generation, revision, the Humanizer, STEM and study tools, upgrade to Pro at $29.99/month — or use Pay-As-You-Go from $1.99 with no subscription at all.",
  },
  {
    q: "Can it answer questions that need current or real-world data, like finance?",
    a: "Yes — and it won't make numbers up. For finance, markets, economics and current-events questions, Light Speed Ghost fetches real live market data (from Yahoo Finance) and grounds the answer in a vetted registry of 1,000+ authoritative sources worldwide — Bloomberg, Reuters, the Financial Times, central banks, stock exchanges, regulators, national statistics offices, the IMF/World Bank/OECD, and regional business press on every continent. The AI is required to cite a named source for any figure, price, rate, date or event, and to tell you plainly when it doesn't have verified current data rather than guess. It never presents out-of-date knowledge as if it were live.",
  },
  {
    q: "Will the checker wrongly flag my own writing as AI or plagiarism?",
    a: "We work hard to prevent that — a false accusation against a student's own work is the worst outcome, so the checker is deliberately conservative. The AI detector needs corroborating signals before it will call anything AI-written, stays silent on very short passages where no detector is reliable, and reports weak signals as human. The plagiarism scan only shows a source as a real match at a confident similarity, so shared topic vocabulary is never flagged. It also understands typos, so a mistyped submission still gets an accurate result. Every check runs locally with no AI model on the Free plan.",
  },
  {
