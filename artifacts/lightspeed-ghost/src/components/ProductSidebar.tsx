...
import {
  PenLine, ListTree, FileEdit, Wand2,
...
export function ProductSidebar() {
  return (
    <aside className="hidden lg:flex fixed top-16 left-0 bottom-0 w-[84px] bg-[#e6f4ec] border-r border-[#cfe8d9] flex-col z-30">
      <div className="flex-1 overflow-y-auto py-3 px-1.5 space-y-0.5">
        {/* Tools — the dashboard/free tool lives at the logo + "/", so no "New"
            button here; every rail item opens its tool (AuthGuard → sign-up). */}
        {TOOL_NAV.map((item) => (
...
export function ProductSidebarDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="lg:hidden fixed inset-0 z-[100]" role="dialog" aria-modal="true">
      ...
      <div className="absolute top-0 left-0 bottom-0 w-72 max-w-[80%] bg-[#e6f4ec] shadow-2xl flex flex-col animate-in slide-in-from-left duration-200">
        <div className="flex items-center justify-between h-16 px-4 border-b border-[#cfe8d9] shrink-0">
          <Logo size={26} textSize="text-sm" variant="light" />
          <button onClick={onClose} aria-label="Close menu" className="p-2 rounded-lg text-[#45464d] hover:bg-[#daeee2] transition-colors">
            <X size={18} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-3 space-y-1">
          <p className="px-3 pt-2 pb-1 text-[10px] font-bold uppercase tracking-widest text-[#76777d]">Tools</p>
